...
// 定義 findStructure 函數來處理 Donor 或 Acceptor 的 SMILES 或名稱輸入
async function findStructure(input, type) {
    ...
    function isSmiles(input) {
        ...
        return smilesPattern.test(input);
    }
    ...
    if (!smiles && !isSmiles(input)) {
        smiles = await convertNameToSmiles(input); // 調用 API 將名稱轉換為 SMILES
    }

    // 3. 如果 API 轉換失敗，將輸入直接當作 SMILES
    if (!smiles) {
        if (isSmiles(input)) {
            smiles = input; // 輸入的是有效的 SMILES 字符串
        } else {
...
// 綁定按鈕事件，每次點擊都重新獲取 donorInput 和 acceptorInput 的資料
document.getElementById('nameToStructureBtn').addEventListener('click', async function() {
    ...
    // 處理 Donor 和 Acceptor
    if (donor) {
        await findStructure(donor, 'donor'); // 呼叫 findStructure 處理 Donor
    }

    if (acceptor) {
        await findStructure(acceptor, 'acceptor'); // 呼叫 findStructure 處理 Acceptor
    }
...
document.getElementById('propertiesButton').addEventListener('click', async function() {
    ...
    // 處理 Donor 和 Acceptor
    if (donor) {
        await findProperties(donor, 'donor'); // 呼叫 findStructure 處理 Donor
    }

    if (acceptor) {
        await findProperties(acceptor, 'acceptor'); // 呼叫 findStructure 處理 Acceptor
    }
...
document.getElementById('performanceButton').addEventListener('click', async function() {
    ...
    // 處理 Donor 和 Acceptor
    if (donor) {
        await findPerformance(donor, 'donor'); // 呼叫 findStructure 處理 Donor
    }

    if (acceptor) {
        await findPerformance(acceptor, 'acceptor'); // 呼叫 findStructure 處理 Acceptor
    }
